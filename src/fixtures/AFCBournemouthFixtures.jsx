...
const AFCBournemouthFixtures = () => {
  ...
  const homeFixtures = allPremierLeagueFixtures.filter((fixture) => {
    return fixture["home-team"].name === "AFC Bournemouth";
  });

  const awayFixtures = allPremierLeagueFixtures.filter((fixture) => {
    return fixture["away-team"].name === "AFC Bournemouth";
  });
  ...
  const getPremierLeagueFixtures = async () => {
    const url =
      "https://football-web-pages1.p.rapidapi.com/fixtures-results.json?comp=1&round=1&team=47";
...
export default AFCBournemouthFixtures;
