...
const NottinghamForestFixtures = () => {
  ...
  const homeFixtures = allPremierLeagueFixtures.filter((fixture) => {
    return fixture["home-team"].name === "Nottingham Forest";
  });

  const awayFixtures = allPremierLeagueFixtures.filter((fixture) => {
    return fixture["away-team"].name === "Nottingham Forest";
  });
  ...
  const getPremierLeagueFixtures = async () => {
    const url =
      "https://football-web-pages1.p.rapidapi.com/fixtures-results.json?comp=1&round=1&team=32";
...
export default NottinghamForestFixtures;
